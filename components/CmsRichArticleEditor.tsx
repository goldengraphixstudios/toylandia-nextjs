...
function normalizeStoredSrc(src: string) {
  return src.replace(/^\/toylandia-nextjs(?=\/)/, "");
}
  ...
  return (
    <button
      ...
      className={`rounded-lg px-3 py-2 text-xs font-black transition-colors ${
        active ? "bg-tl-yellow text-tl-ink" : "text-slate-600 hover:bg-slate-100"
      }`}
    >
      {children}
    </button>
...
function Toolbar({ editor }: { editor: Editor }) {
  function insertImage() {
    const url = window.prompt("Image path or URL", "/toy-1.jpg");
    if (!url) {
      return;
    }

    const alt = window.prompt("Image alt text", "ToyLandia toy stock") ?? "";
  ...
  return (
    <div className="flex flex-wrap items-center gap-1 border-b border-slate-200 bg-white px-3 py-2">
      ...
      <ToolbarButton title="Link" active={editor.isActive("link")} onClick={editLink}>
        Link
      </ToolbarButton>
...
export default function CmsRichArticleEditor({ sections, onChange }: Props) {
  ...
  return (
    <div className={expanded ? "fixed inset-0 z-[100] flex flex-col bg-white" : "overflow-hidden rounded-2xl border-2 border-slate-200 bg-white"}>
      <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50">
        <Toolbar editor={editor} />
        <button
          type="button"
          onClick={() => setExpanded((current) => !current)}
          className="mr-3 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-600 hover:text-slate-950"
        >
          {expanded ? "Collapse" : "Expand"}
        </button>
      </div>
      <div className={expanded ? "flex-1 overflow-y-auto" : "max-h-[760px] overflow-y-auto"}>
